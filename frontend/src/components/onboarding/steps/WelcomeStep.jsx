...
const WelcomeStep = ({ onNext, onSkip }) => {
  ...
  return (
    <div className="text-center max-w-3xl mx-auto">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-gradient-to-b from-sage-100/30 via-sage-50/20 to-transparent rounded-full blur-3xl pointer-events-none dark:hidden" />

      {/* Dark mode icon */}
      <div className="hidden dark:block mb-6">
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" className="mx-auto text-sage-400">
            ...
            <React.Fragment key={step.label}>
              <div className="flex flex-col items-center">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-2 ${
                  step.active ? 'bg-sage-50 text-sage-600' : 'bg-muted text-muted-foreground'
                }`}>
      ...
      <div className="hidden dark:block">
        <div className="inline-flex items-center gap-1 p-1.5 bg-card rounded-full border border-border">
          {steps.map((step, index) => (
            <div
              key={step.label}

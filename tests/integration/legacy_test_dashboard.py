...
def test_get_dashboard_summary(client, db):
    # Setup
    ...
    response = client.get("/api/dashboard/summary?portfolio_id=p1")
    ...
    response_default = client.get("/api/dashboard/summary")
    assert response_default.status_code == 200
    assert response_default.json()["net_worth"] == 105000

def test_dashboard_history_and_snapshot(client, db):
    # Setup
    db.portfolios.data = [{"id": "p1", "user_id": "dev-user-01"}]

    # Create snapshot
    response = client.post("/api/dashboard/snapshot?portfolio_id=p1")
    assert response.status_code == 200
    assert "snapshot" in response.json()

    # Get history
    response_hist = client.get("/api/dashboard/net-worth-history?portfolio_id=p1")
